...
class TestRestImportCommand(TestCase):
    ...
    def test_rest_import_with_renamed_source(self):
        ...
        call_command('rest_import', url, rename_data_source=rename_data_source)

        data_source_model = get_data_source_model()
        qs = data_source_model.objects.filter(name__in=['new-source-1', 'new-source-2'])
        self.assertTrue(qs.count(), 2)
        self.assertFalse(data_source_model.objects.filter(name__in=['test-source-1', 'test-source-2']).exists())

    def test_rest_import_with_invalid_renamed_source(self):
        url = 'http://fake.url/organizations/?page=1'
        rename_data_source = ['test-source-1: new-source-1', 'test-source-2, new-source-2']
        self.assertRaises(CommandError, call_command, 'rest_import', url, rename_data_source=rename_data_source)
